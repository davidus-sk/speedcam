...
foreach ($cams as $cam) {
	...
	if (empty($pid)) {
		...
		`/usr/bin/ffmpeg -hide_banner -rtsp_transport udp -fflags discardcorrupt -flags low_delay -r 15 -i {$cam['url']} -f segment -segment_time 5 -reset_timestamps 1 -strftime 1 -c copy "/data/vms/{$cam['camera']}_%Y-%m-%d_%H-%M-%S.mp4" > /dev/null 2>&1 &`;
	}//if
}//foreach